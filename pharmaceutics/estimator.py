...
def estimateMolecule(m):
    ...
    for i in COOHindices:
        est = BFestimate(m, i, EWGindices, 4.8)
        if(est < mostAcidic[1]):
            mostAcidic = [i, est, "Acid"]
...
def smilesToEst(smiles):
    ...
    return(estimateMolecule(Chem.MolFromSmiles(smiles)))
